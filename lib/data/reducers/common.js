...
var asyncReducer = function asyncReducer(name) {
  return function () {
    ...
    var newDataFlavor = flavor.length > 1 ? _defineProperty({}, flavor[1], action.data) : action.data; // The action timestamp

    var timestamp = action.timestamp; // The data provider

    var provider = action.provider; // Let's work on the new state
    ...
    switch (actionState.toLowerCase()) {
      ...
      case 'error':
        ...
        return Object.assign({}, newState, Object.keys(error).length > 0 ? {
          error: error
        } : {});
      ...
      case 'ok':
        ...
        return Object.assign({}, newState, Object.keys(data).length > 0 ? {
          data: data
        } : {});
