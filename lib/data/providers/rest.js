...
var RestDataProvider = function (_DataProvider) {
  ...
  _createClass(RestDataProvider, [{
    ...
    value: function _prepareAuthHeaders(auth) {
      ...
      var access = Object.assign({}, auth.access, { email: auth.user.email, id: auth.user._id });

      return {
        Authorization: 'Bearer ' + _jsBase.Base64.encode('' + JSON.stringify(access))
      };
    }
  }, {
    key: '_prepareRequest',
    value: function _prepareRequest(_ref3, auth) {
      ...
      var authHeaders = this._prepareAuthHeaders(auth);
    ...
    value: function _sendAuthRequest(request, auth) {
      var requestParams = this._prepareRequest(request, auth);
    ...
    value: function _sendRequest(request) {
      ...
      return (0, _cache.retrieveAuth)().then(function (auth) {
        return _this2._sendAuthRequest(request, auth);
      }).catch(function () {
        return _this2._sendAuthRequest(request);
      });
    }
  }, {
    key: 'defaults',
